...
terraform output DPSNonprodServiceAccount_encrypted_aws_secret_access_key | sed 's/"//g' | base64 -d | gpg -dq --passphrase ${GPG_KEY_PASSPHRASE} |  opw write team9-aws DPSNonprodServiceAccount-aws-secret-access-key -
terraform output DPSNonprodServiceAccount_aws_access_key_id | tr -d \\n | sed 's/"//g' | opw write team9-aws DPSNonprodServiceAccount-aws-access-key-id -

# write the prod service account credentials to secret store
terraform output DPSProdServiceAccount_encrypted_aws_secret_access_key | sed 's/"//g' | base64 -d | gpg -dq --passphrase ${GPG_KEY_PASSPHRASE} |  opw write team9-aws DPSProdServiceAccount-aws-secret-access-key -
terraform output DPSProdServiceAccount_aws_access_key_id | tr -d \\n | sed 's/"//g' | opw write team9-aws DPSProdServiceAccount-aws-access-key-id -
